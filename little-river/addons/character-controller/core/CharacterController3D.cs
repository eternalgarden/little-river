...
public partial class CharacterController3D : CharacterBody3D
{
    ...
    [Signal]
    public delegate void FlyModeActivedEventHandler();

    // Emitted when a fly mode is finished, called when [FlyModeAbility3D] is
    // deactive.
    [Signal]
    public delegate void FlyModeDeactivedEventHandler();
    ...
    [Export]
    public float StepLengthen = 0.7f;
    ...
    [Export]
    public float HeightInCrouch = 1f;
    ...
    [Export]
    public int FlyModeSpeedModifier = 2;
    ...
    [Export]
    public float FloatingHeight = 0.75f;
    ...
    [Export]
    public Array<NodePath> AbilitiesPath = new Array<NodePath>();
    ...
    public virtual void Move(
        ...
        bool inputSwimDown = false,
        bool inputSwimUp = false
    )
    {
        Vector3 direction = DirectionInput(
            inputAxis,
            inputSwimDown,
            inputSwimUp,
            _directionBaseNode
        ...
        CrouchAbility.SetActive(
            inputCrouch && IsOnFloor() && !IsFloating() && !IsSubmerged() && !IsFlyMode()
        );
        SprintAbility.SetActive(
            inputSprint
                && IsOnFloor()
                && inputAxis.X >= 0.5
                && !IsCrouching()
                && !IsFlyMode()
                && !SwimAbility.IsFloating()
                && !SwimAbility.IsSubmerged()
        );

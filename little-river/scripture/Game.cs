...
{
	[Export] Node3D _activeSceneParent;

	[Export] PackedScene _mainMenuScene;
	[Export] PackedScene _levelOneScene;
		...
		Q += rzeka.Loom<GameOpened, MainMenuLoaded>(
			...
			spell => spell.SelectMany(gameStarted => rzeka
				.Ask<LoadSceneRequest, LoadSceneResponse>(
					this,
					new LoadSceneRequest(_mainMenuScene.ResourcePath)
						.WithCircumstances(gameStarted))
				.Take(1)
				...
				.Select(r => 
					new MainMenuLoaded().WithCircumstances(gameStarted, r))));

		// Q += rzeka.Loom<MainMenuLoaded, GameReady>(
		// 	this,
		// 	spell => spell
		// 		.Take(1)
		// 		.Select(_ => new GameReady()));
	}
}
